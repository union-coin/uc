...
const QString BITCOIN_IPC_PREFIX("UnionCoin:");
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click UnionCoin: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start UnionCoin: click-to-pay handler");
    else
...
{
    // clicking on UnionCoin: URLs creates FileOpen events on the Mac:
